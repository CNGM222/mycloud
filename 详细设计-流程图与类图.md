...
flowchart TD
    ...
    H --> I[开启数据库事务]
    I --> J[插入 files 元数据]
    J --> K[JDBC setBinaryStream 写入 LONGBLOB]
    K --> L[回写 files.content_id]
    L --> M[更新 users.used_capacity]
    M --> N[任务改为 success]
    N --> O[写入 upload 日志]
    O --> P[提交事务并返回文件信息]
    I --> Q{任一步失败}
    Q -->|是| R[回滚事务并标记任务 failed]
```
...
flowchart TD
    ...
    E -->|是| F[创建下载任务]
    F --> G[设置 HTTP 下载响应头]
    G --> H[JDBC getBinaryStream 读取 LONGBLOB]
    H --> I[分块写入响应输出流]
    I --> J[任务改为 success]
    J --> K[写入 download 日志]
```
...
flowchart TD
    ...
    G --> H
    H --> I[开启事务]
    I --> J[删除 file_contents]
    J --> K[子节点优先删除 files]
    K --> L[更新 users.used_capacity]
    L --> M[写入 delete 日志]
    M --> N[提交事务]
```
...
classDiagram
    ...
    class FileDao {
        ...
        +updateContentId(Long, Long) void
    ...
    class FileContentDao {
        +insertContent(Long, InputStream, long, String) long
        +openContentStream(Long) InputStream
        +deleteByFileIds(List) void
    }
...
classDiagram
    ...
    FileEntity "1" --> "0..1" FileContentEntity
